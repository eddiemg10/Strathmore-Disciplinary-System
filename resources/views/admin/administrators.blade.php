...
@section('sidebar')
<x-admin-sidebar focus="admin" />
@endsection
@section('content')

<div class="flex flex-col items-center pb-40 px-5 md:px-20 gap-y-20">

    <x-admin-registration title="System Administrator Registration" btn="Register Administrator" />

    <div class="bg-white px-10 py-5 shadow-md mt-10 w-full rounded-md flex flex-col items-center gap-y-8">
        <h1 class="text-zinc-700 md:text-3xl text-2xl font-semibold">System Administrator Management</h1>
        <button id="add-teacher"
            ...
            New System Administrators to the System</button>
        <div class="w-full">
            <x-session-messages />
        </div>
    </div>
    ...
    <div class="flex flex-wrap lg:flex-nowrap gap-x-5  sm:gap-y-10 w-full">
        ...
        <x-admin-searchbar title="Find Administrator" title2="Staff Number" type="admin" />
...
<script>
    $( document ).ready(function() {
        ...
        $("#teachers-table").on('click','tr', function(){
            ...
            $.get("/admin/administrators/"+id, function(data, status){
